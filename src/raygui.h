...
bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            {
                [[maybe_unused]] float maxWidth = (bounds.width - (GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING)*2));
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            int byteSize = 0;
            [[maybe_unused]] const char *textUTF8 = CodepointToUTF8(codepoint, &byteSize);
...
// Color Panel control
Color GuiColorPanel(Rectangle bounds, [[maybe_unused]] const char *text, Color color)
{
...
float GuiColorBarAlpha(Rectangle bounds, [[maybe_unused]] const char *text, float alpha)
{
...
float GuiColorBarHue(Rectangle bounds, [[maybe_unused]] const char *text, float hue)
{
    GuiState state = guiState;
...
Color GuiColorPicker(Rectangle bounds, [[maybe_unused]] const char *text, Color color)
{
...
Vector2 GuiGrid(Rectangle bounds, [[maybe_unused]] const char *text, float spacing, int subdivs)
{
